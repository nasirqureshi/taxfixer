...
public class MainRun {
	...
	//public static String targedb = "chtaxdb";
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (rsAdjust.next()) {
				...
				System.out.println("STARTDATE:" + " " + adjustdatefrom
						+ " ENDDATE:" + adjustdateto + "  TARGET:"
						+ targetamountadjust + "  additional:"
						+ additionaladjust);
				...
				additionalinvoiceList = getAllMatchedAdditionalInvoices(
						adjustdatefrom, adjustdateto);
				if (!(additionaladjust == 0.00)) {
					if (additionaladjust > 0) {

						processAdditionalAdjustPlus(additionalinvoiceList,
								additionaladjust, true);

					}
					else {
						processAdditionalAdjustMinus(additionalinvoiceList,
								additionaladjust, false);
					}
				}
			}
	...
	private static Map<Integer, Integer> getAllMatchedAdditionalInvoices(
			Date adjustdatefrom, Date adjustdateto) {
		Map<Integer, Integer> additionalinvoiceList = new LinkedHashMap<Integer, Integer>();
		String sqlMatchInvoices = "SELECT i.invoicenumber , COUNT(id.InvoiceNumber) cnt FROM invoice i, invoicedetails id "
				+ " WHERE i.InvoiceNumber = id.InvoiceNumber "
		...
		try {
			...
			while (rsAdjust.next()) {
				additionalinvoiceList.put(rsAdjust.getInt("invoicenumber"),
						rsAdjust.getInt("cnt") - 1);
	...
	private static void processAdditionalAdjustMinus(
			Map<Integer, Integer> additionalinvoiceList, Float additionaladjust,
			boolean indice) {
		...
		try {
			...
			for (Map.Entry<Integer, Integer> entry : additionalinvoiceList
					.entrySet()) {
				...
				if (finaladditionaladjust < additionaladjust) {
					pstmtAdditional = connection
							.prepareStatement(sqlAdditional);
					...
					while (rsAdditional.next()) {
						...
						ida.setAddddetailsamount(
								rsAdditional.getFloat("addamount"));
						invoicedetailslist.add(ida);
						invoiceadditionaladjust = invoiceadditionaladjust
								+ rsAdditional.getFloat("addamount");

						if (finaladditionaladjust > additionaladjust) {
							System.out.println(finaladditionaladjust + " ^ "
									+ additionaladjust);
							break;
						}
					...
					System.out.println(
							finaladditionaladjust + " ^ " + additionaladjust);
					break;
				}
				ia.setInvoicenumber(entry.getKey());
				ia.setAdjustamount(invoiceadditionaladjust);
				invoicelist.add(ia);
				finaladditionaladjust = finaladditionaladjust
						+ invoiceadditionaladjust;
			}
	...
	private static void updateInvoiceDetails(
			List<AdditionalAdjustInvoiceDetails> invoicedetailslist,
			boolean index) {
		...
		try {
			...
			for (AdditionalAdjustInvoiceDetails taxmatrix : invoicedetailslist) {
				...
				pstmtupdateInvoiceDetails = connection
						.prepareStatement(updateInvoiceDetailsSQL);
				pstmtupdateInvoiceDetails.setFloat(1,
						taxmatrix.getAddddetailsamount());
				pstmtupdateInvoiceDetails.setInt(2,
						taxmatrix.getInvoicenumber());
				pstmtupdateInvoiceDetails.setString(3,
						taxmatrix.getPartnumber());
	...
	private static void processAdditionalAdjustPlus(
			Map<Integer, Integer> additionalinvoiceList, Float additionaladjust,
			boolean indice) {
		Float finaladditionaladjust = 0.00f;
		Float invoiceadditionaladjust = 0.00f;
	...
	private static void updateInvoice(List<AdditionalAdjustInvoice> invoicelist,
			boolean index) {
		...
		try {
			...
			for (AdditionalAdjustInvoice ainv : invoicelist) {
				...
				pstmtUpdateInvoice = connection
						.prepareStatement(updateInvoiceSQL);
	...
	private static void processEntries(Map<Integer, Integer> invoiceList,
			List<String> customerList, Float targetamount) {
			...
			for (Map.Entry<Integer, Integer> entry : invoiceList.entrySet()) {
				if (finalamountadjust < targetamount) {
					...
					while (rsTaxer.next()) {
						...
						finalamountadjust = finalamountadjust
								+ rsTaxer.getFloat("taxremovedamount");
						taxadjustmatrix.setInvoicenumber(entry.getKey());
						taxadjustmatrix
								.setPartnumber(rsTaxer.getString("partnumber"));
						taxadjustmatrix.setTaxremovedamount(
								rsTaxer.getFloat("taxremovedamount"));
						taxadjustmatrix
								.setSoldPrice(rsTaxer.getFloat("soldprice"));
						...
						if (finalamountadjust > targetamount) {
							...
							updateFinalTaxNumbers(adjustmatrixlist,
									customerList);
	...
	private static void updateFinalTaxNumbers(
			List<TaxAdjustMatrix> adjustmatrixlist, List<String> customerList) {
		String updateInvoiceSQL = "UPDATE invoice SET   Tax = Tax - ?, CustomerID = ? WHERE invoicenumber = ?";
		// String updateInvoiceDetailsSQL = "UPDATE invoicedetails SET soldprice
		// = soldprice + ? WHERE invoicenumber = ? AND PartNumber = ?";
		Connection connection = null;
		PreparedStatement pstmtUpdateInvoice = null;
		...
		try {
			...
			for (TaxAdjustMatrix taxmatrix : adjustmatrixlist) {
				...
				pstmtUpdateInvoice.executeUpdate();
				// pstmtdeleteInvoiceDetails =
				// connection.prepareStatement(updateInvoiceDetailsSQL);
				// pstmtdeleteInvoiceDetails.setFloat(1,
				// taxmatrix.getTaxremovedamount());
				// pstmtdeleteInvoiceDetails.setInt(2,
				// taxmatrix.getInvoicenumber());
				// pstmtdeleteInvoiceDetails.setString(3,
				// taxmatrix.getPartnumber());
	...
	private static List<String> getAllMatchingCustomer(Date adjustdatefrom,
			Date adjustdateto) {

		List<String> customerList = new LinkedList<String>();
		String sqlMatchInvoices = "SELECT i.customerid  FROM invoice i "
	...
	private static Map<Integer, Integer> getAllMatchedInvoices(
			Date adjustdatefrom, Date adjustdateto) {
		Map<Integer, Integer> invoiceList = new LinkedHashMap<Integer, Integer>();
		String sqlMatchInvoices = "SELECT i.invoicenumber , COUNT(id.InvoiceNumber) cnt FROM invoice i, invoicedetails id "
				+ " WHERE i.InvoiceNumber = id.InvoiceNumber "
		...
		try {
			...
			rsAdjust = pstmtAdjust.executeQuery();
			while (rsAdjust.next()) {
				invoiceList.put(rsAdjust.getInt("invoicenumber"),
						rsAdjust.getInt("cnt"));
				// System.out.println(rsAdjust.getInt("invoicenumber"));
			}
